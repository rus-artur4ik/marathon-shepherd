...
ARCHITECTURE_SAMPLES_ARTIFACT_DIR="${ARTIFACT_PARENT}/architecture-samples-${ARCHITECTURE_SAMPLES_REPO_SHA}"
...
PREPARE_ARCHITECTURE_SAMPLES_SCRIPT="${SCRIPT_DIR}/build_test_apk.sh"
...
start_default_emulator() {
    ...
    if [[ "${USE_DYNAMIC_UI}" == "true" ]]; then
        ...
        plain_step "Start emulator ${PUBLIC_UI_AVD_NAME}"
    fi
    ...
    while true; do
        if detected_serial="$(find_connected_emulator_serial)"; then
            if wait_for_device_boot "${detected_serial}"; then
                ...
                if [[ "${USE_DYNAMIC_UI}" == "true" ]]; then
                    ...
                    plain_info "${detected_serial} booted"
                fi
        ...
        if (( elapsed >= 240 )); then
            if [[ "${USE_DYNAMIC_UI}" == "true" ]]; then
                ...
                plain_warn "timed out waiting for ${PUBLIC_UI_AVD_NAME}"
            fi
            return 1
        fi
        if [[ "${USE_DYNAMIC_UI}" == "true" ]]; then
