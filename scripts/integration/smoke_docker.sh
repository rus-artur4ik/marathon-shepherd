...
#
# Usage:
#   scripts/integration/smoke_docker.sh                        # all services
#   MSH_COMPONENT_SERVICE=shepherd-adb   scripts/integration/smoke_docker.sh
#   MSH_COMPONENT_SERVICE=shepherd-farm  scripts/integration/smoke_docker.sh
#   MSH_COMPONENT_SERVICE=shepherd-cuttlefish  scripts/integration/smoke_docker.sh
#   MSH_COMPONENT_SERVICE=manager        scripts/integration/smoke_docker.sh
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
COMPOSE_FILE="${REPO_ROOT}/deploy/docker-compose.smoke.yml"
