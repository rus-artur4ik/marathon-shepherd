...
set_log_context "Real Device Integration"
enable_dynamic_logs
...
SESSION_ID=""
ADAPTER_URL=""
ADAPTER_SECRET="integration-secret-real-device"
...
start_self_managed_stack() {
    ...
    log_step "Preparing temporary real-device manager config (${CONFIG_FILE})"
    printf '%s\n' \
        "providers:" \
    ...
    log_step "Starting temporary manager process on port ${manager_port}"
    env \
    ...
    log_step "Waiting for physical inventory to appear in manager"
...
if [[ -n "${EXTERNAL_MANAGER_URL}" ]]; then
    MANAGER_URL="${EXTERNAL_MANAGER_URL}"
    log_step "Waiting for external manager readiness at ${MANAGER_URL}"
    if ! wait_for_http_json "${MANAGER_URL}/health" 60; then
...
log_step "Scenario 1: provider inventory exposes requested device type (${DEVICE_TYPE})"
http_json "GET" "${MANAGER_URL}/api/v1/devices"
assert_status "200"
...
if [[ -z "${API_LEVEL}" ]]; then
    ...
    if [[ -z "${API_LEVEL}" ]]; then
        fail "Could not resolve apiLevel automatically from provider inventory"
    fi
fi
log_success "Inventory contains requested device type"
log_info "Using apiLevel ${API_LEVEL} for session request"

log_step "Scenario 2: allocate and release a real-device session"
REQUEST_PAYLOAD="$(python3 - <<PY
import json
print(json.dumps({
    "devices": int("${REQUESTED_DEVICES}"),
    "apiLevel": "${API_LEVEL}",
...
SESSION_ID=""
log_success "Allocation and release completed"
